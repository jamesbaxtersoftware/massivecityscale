...
/// right = +x, down = +y. Cursor right turns the nose right; cursor up pitches
/// the nose up. No roll is ever introduced, so world up stays up. Pitch is
/// clamped to ±`MAX_PITCH`; yaw wraps freely.
...
mod tests {
    ...
    fn ship_rotation_neutral_faces_forward_and_upright() {
        ...
        assert!((r * Vec3::NEG_Z - Vec3::NEG_Z).length() < 1e-5, "neutral faces -Z");
        assert!((r * Vec3::Y - Vec3::Y).length() < 1e-5, "neutral is upright");
    }

    #[test]
    fn cursor_up_pitches_nose_up_down_pitches_down() {
        // Cursor up => off.y negative.
        let (y, p) = steer(0.0, 0.0, Vec2::new(0.0, -1.0), 1.0, 0.1);
        assert!((ship_rotation(y, p) * Vec3::NEG_Z).y > 0.0, "nose rises when cursor up");
        let (y, p) = steer(0.0, 0.0, Vec2::new(0.0, 1.0), 1.0, 0.1);
        assert!((ship_rotation(y, p) * Vec3::NEG_Z).y < 0.0, "nose drops when cursor down");
    }

    #[test]
    fn cursor_right_turns_right_left_turns_left() {
        let (y, p) = steer(0.0, 0.0, Vec2::new(1.0, 0.0), 1.0, 0.1);
        assert!((ship_rotation(y, p) * Vec3::NEG_Z).x > 0.0, "nose turns toward +X (right)");
        let (y, p) = steer(0.0, 0.0, Vec2::new(-1.0, 0.0), 1.0, 0.1);
        assert!((ship_rotation(y, p) * Vec3::NEG_Z).x < 0.0, "nose turns toward -X (left)");
    }

    #[test]
    fn pitch_is_clamped() {
        let (_, p) = steer(0.0, 0.0, Vec2::new(0.0, -100.0), 5.0, 1.0);
        assert!(p <= MAX_PITCH + 1e-6 && p >= -MAX_PITCH - 1e-6, "pitch within ±MAX_PITCH");
    }
